...
function AdminWorkspace() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 pb-10"><AdminHeader section={section} />{section === "overview" ? <AdminOverview /> : null}{section === "customers" ? <div className="grid gap-5"><CustomerAccountAdminCard /><AdminBalanceEditor /></div> : null}{section === "deposits" ? <PaymentReviewAdminCard requestType="deposit" /> : null}{section === "withdrawals" ? <PaymentReviewAdminCard requestType="withdrawal" /> : null}{section === "bonuses" ? <div className="grid gap-5"><BonusPolicyAdminCard /><ReferralCommissionAdminCard /></div> : null}{section === "site" ? <SiteConfigurationPage /> : null}{section === "matches" ? <div className="grid gap-5"><AdminMatchCreator /><GamesFeedPreview heading="Scores, fixtures & forecasts" showPredictions /></div> : null}{section === "administrators" ? <AdminManagementCard /> : null}</div>;
}
