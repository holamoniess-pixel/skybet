...
export function SportsPage() {
  ...
  return <CustomerShell activeMobileNav="Sports"><PageHeader eyebrow="Discover sports" title="Upcoming matches" description="Browse football fixtures generated by the live match service. Pairings, dates, forecasts, and odds are internally managed markets." /><main className="container py-5 md:py-6"><div className="mb-3 grid gap-2 sm:grid-cols-3">{["Football", "Tennis", "Virtuals"].map((sport, index) => <Card key={sport} className="border-[var(--sky-blue-100)] bg-white dark:border-white/10 dark:bg-[var(--card)]"><CardContent className="flex items-center gap-2.5 p-2.5"><span className="grid size-8 place-items-center rounded-lg bg-[var(--sky-ice-100)] text-[var(--sky-blue-600)]"><Trophy className="size-3.5" /></span><div><p className="text-[13px] font-extrabold text-[var(--sky-navy-950)] dark:text-white">{sport}</p><p className="mt-px text-[11px] text-[var(--sky-navy-600)] dark:text-slate-400">{index === 0 ? "Generated fixtures" : "Catalogue reserved"}</p></div></CardContent></Card>)}</div><EventRows events={events} /></main></CustomerShell>;
}

export function GamesPage() {
  return <CustomerShell activeMobileNav="Sports"><PageHeader eyebrow="Games hub" title="SKYBET match centre" description="View SKYBET football pairings, upcoming/live states, forecasts, scores, and market odds." /><main className="container py-5 md:py-6"><GamesFeedPreview heading="Scores and fixtures" /><Card className="mt-3 border-[var(--sky-blue-100)] bg-[var(--sky-navy-950)] text-white dark:border-white/10"><CardContent className="flex items-center gap-3 p-3"><Gamepad2 className="size-5 text-[var(--sky-blue-300)]" /><div><p className="text-sm font-extrabold">Market information</p><p className="mt-1 text-xs leading-5 text-slate-300">Pairings, dates, scores, forecasts, and market odds are managed by SKYBET.</p></div></CardContent></Card></main></CustomerShell>;
}
