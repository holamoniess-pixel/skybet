...
export function SportsPage() {
  ...
  return <CustomerShell activeMobileNav="Sports"><PageHeader eyebrow="Discover sports" title="Upcoming events" description="Browse original SKYBET preview cards across football, tennis, virtuals, and future catalogue categories." /><main className="container py-5 md:py-6"><div className="mb-4 grid gap-2 sm:grid-cols-3">{["Football", "Tennis", "Virtuals"].map((sport, index) => <Card key={sport} className="border-[var(--sky-blue-100)] bg-white dark:border-white/10 dark:bg-[var(--card)]"><CardContent className="flex items-center gap-3 p-3"><span className="grid size-9 place-items-center rounded-xl bg-[var(--sky-ice-100)] text-[var(--sky-blue-600)]"><Trophy className="size-4" /></span><div><p className="text-sm font-extrabold text-[var(--sky-navy-950)] dark:text-white">{sport}</p><p className="mt-0.5 text-xs text-[var(--sky-navy-600)] dark:text-slate-400">{index === 0 ? "Featured preview events" : "Catalogue preview"}</p></div></CardContent></Card>)}</div><EventRows events={events} /></main></CustomerShell>;
}
...
export function GamesPage() {
  ...
  return <CustomerShell activeMobileNav="Sports"><PageHeader eyebrow="Provider sandbox" title="Games feed preview" description="This page demonstrates how normalized match data can render before a licensed provider integration is approved." /><main className="container py-5 md:py-6"><GamesFeedPreview onMarketSelect={chooseSelection} /><Card className="mt-4 border-[var(--sky-blue-100)] bg-[var(--sky-navy-950)] text-white dark:border-white/10"><CardContent className="flex items-center gap-3 p-4"><Gamepad2 className="size-6 text-[var(--sky-blue-300)]" /><div><p className="font-extrabold">Simulated feed boundary</p><p className="mt-1 text-sm leading-5 text-slate-300">The automatic refresh is a client preview of a mock API. It is not a provider connection and does not accept wagers, deposits, or payouts.</p></div></CardContent></Card></main><PreviewSlipFab selection={selection} onOpen={() => setSelectionOpen(true)} /><SelectionSheet open={selectionOpen} onOpenChange={setSelectionOpen} selection={selection} /></CustomerShell>;
}
...
export function ActivityPage() {
  ...
  const copyReferralLink = async () => {
    ...
    try {
      ...
      setCopyMessage("Preview referral link copied.");
    } catch {
      setCopyMessage("We could not copy the link. You can select it manually instead.");
    }
  };

  return <CustomerShell activeMobileNav="Rewards"><PageHeader eyebrow="My activity" title="Preview activity" description="Keep future saved selections, account updates, and safer-play actions visible in one original SKYBET view." /><main className="container py-5 md:py-6"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><Card id="rewards" className="border-[var(--sky-blue-100)] bg-white dark:border-white/10 dark:bg-[var(--card)]"><CardContent className="p-4"><ClipboardList className="size-5 text-[var(--sky-blue-600)]" /><h2 className="mt-3 text-base font-extrabold text-[var(--sky-navy-950)] dark:text-white">Preview activity</h2><p className="mt-1 text-sm leading-5 text-[var(--sky-navy-600)] dark:text-slate-400">Selections remain local to this preview session.</p></CardContent></Card><Card className="border-[var(--sky-blue-100)] bg-[var(--sky-ice-50)] dark:border-white/10 dark:bg-white/5"><CardContent className="p-4"><ShieldCheck className="size-5 text-[var(--sky-emerald-700)] dark:text-[var(--sky-emerald-500)]" /><h2 className="mt-3 text-base font-extrabold text-[var(--sky-navy-950)] dark:text-white">Account controls</h2><p className="mt-1 text-sm leading-5 text-[var(--sky-navy-600)] dark:text-slate-400">Security, support, and safer-play choices stay visible.</p></CardContent></Card><Card className="border-[var(--sky-blue-100)] bg-[var(--sky-navy-950)] text-white dark:border-white/10"><CardContent className="p-4"><Link2 className="size-5 text-[var(--sky-emerald-500)]" /><h2 className="mt-3 text-base font-extrabold">Preview referral link</h2><p className="mt-1 text-xs leading-5 text-slate-300">This link is displayed for interface testing only. Referral rewards are not credited in the preview.</p><code className="mt-3 block truncate rounded-lg bg-white/10 px-2 py-2 text-[11px] text-[var(--sky-blue-200)]">{referralLink}</code><Button type="button" variant="outline" className="mt-3 min-h-10 border-white/20 bg-white/5 text-xs font-extrabold text-white hover:bg-white/10" onClick={copyReferralLink}><Copy className="size-3.5" />Copy preview referral link</Button><p aria-live="polite" className="mt-2 min-h-4 text-xs font-semibold text-[var(--sky-emerald-500)]">{copyMessage}</p></CardContent></Card></div></main></CustomerShell>;
}
...
export function AccountPage() {
  ...
  return <CustomerShell activeMobileNav="Account"><PageHeader eyebrow="Account centre" title="Your SKYBET controls" description="A clear account preview with safety, identity, and support areas ready for approved integrations." /><main className="container py-5 md:py-6"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{accountSections.map(({ id, icon: Icon, title, text, email }) => <Card id={id} key={title} className="border-[var(--sky-blue-100)] bg-white dark:border-white/10 dark:bg-[var(--card)]"><CardContent className="p-4"><span className="grid size-9 place-items-center rounded-xl bg-[var(--sky-ice-100)] text-[var(--sky-blue-700)]"><Icon className="size-4" /></span><h2 className="mt-3 text-base font-extrabold text-[var(--sky-navy-950)] dark:text-white">{title}</h2><p className="mt-1 text-sm leading-5 text-[var(--sky-navy-600)] dark:text-slate-400">{text}</p>{email ? <a href={`mailto:${email}`} aria-label="Email SKYBET customer service" className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-xl border border-[var(--sky-emerald-600)]/25 bg-[var(--sky-emerald-600)]/10 px-3 text-xs font-extrabold text-[var(--sky-emerald-700)] hover:bg-[var(--sky-emerald-600)]/15 dark:text-[var(--sky-emerald-500)]"><Mail className="size-4" />{email}</a> : null}</CardContent></Card>)}</div></main></CustomerShell>;
}
