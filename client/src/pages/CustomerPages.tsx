...
export function ActivityPage() {
  return <CustomerShell activeMobileNav="Rewards"><PageHeader eyebrow="My activity" title="Preview activity" description="Keep future saved selections, account updates, and safer-play actions visible in one original Skybet view." /><main className="container py-6 md:py-8"><div className="grid gap-4 md:grid-cols-2"><Card className="border-[var(--sky-blue-100)] bg-white dark:border-white/10 dark:bg-[var(--card)]"><CardContent className="p-5"><ClipboardList className="size-6 text-[var(--sky-blue-600)]" /><h2 className="mt-4 text-lg font-extrabold text-[var(--sky-navy-950)] dark:text-white">No saved preview selections</h2><p className="mt-2 text-sm leading-6 text-[var(--sky-navy-600)] dark:text-slate-400">Event selections remain local to the current preview. Account activity will be available once customer features are approved.</p></CardContent></Card><Card className="border-[var(--sky-blue-100)] bg-[var(--sky-ice-50)] dark:border-white/10 dark:bg-white/5"><CardContent className="p-5"><ShieldCheck className="size-6 text-[var(--sky-emerald-700)] dark:text-[var(--sky-emerald-500)]" /><h2 className="mt-4 text-lg font-extrabold text-[var(--sky-navy-950)] dark:text-white">Account controls stay visible</h2><p className="mt-2 text-sm leading-6 text-[var(--sky-navy-600)] dark:text-slate-400">Use the account area to preview security, support, and safer-play choices.</p></CardContent></Card></div></main></CustomerShell>;
}

export function AccountPage() {
  return <CustomerShell activeMobileNav="Account"><PageHeader eyebrow="Account centre" title="Your Skybet controls" description="A clear account preview with safety, identity, and support areas ready for approved integrations." /><main className="container py-6 md:py-8"><div className="grid gap-4 md:grid-cols-3">{[{ icon: UserRound, title: "Profile", text: "Identity and preferences will be visible here after account onboarding." }, { icon: ShieldCheck, title: "Safer play", text: "Limits, exclusion, and support pathways must be server-enforced before launch." }, { icon: CircleHelp, title: "Support", text: "Help and issue-management channels will be connected after compliance approval." }].map(({ icon: Icon, title, text }) => <Card key={title} className="border-[var(--sky-blue-100)] bg-white dark:border-white/10 dark:bg-[var(--card)]"><CardContent className="p-5"><span className="grid size-10 place-items-center rounded-xl bg-[var(--sky-ice-100)] text-[var(--sky-blue-700)]"><Icon className="size-5" /></span><h2 className="mt-5 text-lg font-extrabold text-[var(--sky-navy-950)] dark:text-white">{title}</h2><p className="mt-2 text-sm leading-6 text-[var(--sky-navy-600)] dark:text-slate-400">{text}</p></CardContent></Card>)}</div></main></CustomerShell>;
}
...
export function EventDetailPage() {
  ...
  return <CustomerShell activeMobileNav={event.isLive ? "Live" : "Sports"}><PageHeader eyebrow={event.isLive ? "Live event preview" : "Event preview"} title={`${event.teams[0]} vs ${event.teams[1]}`} description={`${event.competition} · ${event.status}`} /><main className="container py-6 md:py-8"><Button variant="ghost" className="mb-5 -ml-3 font-bold text-[var(--sky-blue-700)]" onClick={() => setLocation(event.isLive ? "/live" : "/sports")}><ArrowLeft className="size-4" /> Return to event board</Button><Card className="overflow-hidden border-[var(--sky-blue-100)] bg-white dark:border-white/10 dark:bg-[var(--card)]"><CardContent className="p-5 sm:p-7"><div className="flex items-center justify-between gap-3"><Badge className="bg-[var(--sky-ice-100)] text-[var(--sky-blue-700)] hover:bg-[var(--sky-ice-100)]">{event.sport}</Badge><span className="text-sm font-extrabold text-[var(--sky-blue-700)] dark:text-[var(--sky-blue-300)]">{event.startsAt}</span></div><div className="mt-8 grid items-center gap-4 text-center sm:grid-cols-[1fr_auto_1fr]"><p className="text-xl font-extrabold text-[var(--sky-navy-950)] dark:text-white">{event.teams[0]}</p><span className="rounded-xl bg-[var(--sky-navy-950)] px-4 py-2 text-lg font-extrabold text-white">{event.score ?? "VS"}</span><p className="text-xl font-extrabold text-[var(--sky-navy-950)] dark:text-white">{event.teams[1]}</p></div><div className="mt-8 grid gap-3 sm:grid-cols-3">{event.markets.map(market => <button key={market.label} type="button" onClick={() => undefined} className="rounded-xl border border-[var(--sky-blue-100)] bg-[var(--sky-ice-50)] p-4 text-left transition hover:border-[var(--sky-blue-300)] dark:border-white/10 dark:bg-white/5"><span className="block text-xs font-bold text-[var(--sky-navy-600)] dark:text-slate-400">{market.label}</span><span className="mt-2 block text-lg font-extrabold text-[var(--sky-blue-700)] dark:text-[var(--sky-blue-300)]">{market.value}</span></button>)}</div><div className="mt-6 rounded-xl bg-[var(--sky-emerald-600)]/10 p-4 text-sm leading-6 text-[var(--sky-emerald-700)] dark:text-[var(--sky-emerald-500)]">This is a non-transactional event preview. Skybet does not currently accept deposits, wagers, or payouts.</div></CardContent></Card></main></CustomerShell>;
}
