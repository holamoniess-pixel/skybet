...
export function LivePage() {
  const feed = trpc.games.simulatedFeed.useQuery(undefined, { refetchInterval: 30_000, refetchIntervalInBackground: false });
  const events = filterSkybetEvents(feed.data?.events ?? [], "live", "All");
  return <CustomerShell activeMobileNav="Live"><PageHeader eyebrow="Live board" title="Live simulated matches" description="Live states and scores generated by the SKYBET simulation engine. This is not a real sports feed." /><main className="container py-5 md:py-6"><div className="mb-3 flex items-center gap-2 rounded-xl border border-[var(--sky-emerald-600)]/25 bg-[var(--sky-emerald-600)]/10 px-3 py-2 text-sm font-bold text-[var(--sky-emerald-700)] dark:text-[var(--sky-emerald-500)]"><Activity className="size-4" /> {events.length} simulated live events</div><EventRows events={events} /></main></CustomerShell>;
}

export function SportsPage() {
  const feed = trpc.games.simulatedFeed.useQuery(undefined, { refetchInterval: 30_000, refetchIntervalInBackground: false });
  const events = filterSkybetEvents(feed.data?.events ?? [], "upcoming", "All");
  return <CustomerShell activeMobileNav="Sports"><PageHeader eyebrow="Discover sports" title="Upcoming SKYBET-generated matches" description="Browse football fixtures generated by the SKYBET internal market engine. Pairings, dates, forecasts, and odds are internally managed markets." /><main className="container py-5 md:py-6"><div className="mb-3 grid gap-2 sm:grid-cols-3">{["Football", "Tennis", "Virtuals"].map((sport, index) => <Card key={sport} className="border-[var(--sky-blue-100)] bg-white dark:border-white/10 dark:bg-[var(--card)]"><CardContent className="flex items-center gap-2.5 p-2.5"><span className="grid size-8 place-items-center rounded-lg bg-[var(--sky-ice-100)] text-[var(--sky-blue-600)]"><Trophy className="size-3.5" /></span><div><p className="text-[13px] font-extrabold text-[var(--sky-navy-950)] dark:text-white">{sport}</p><p className="mt-px text-[11px] text-[var(--sky-navy-600)] dark:text-slate-400">{index === 0 ? "Generated fixtures" : "Catalogue reserved"}</p></div></CardContent></Card>)}</div><EventRows events={events} /></main></CustomerShell>;
}

export function GamesPage() {
  return <CustomerShell activeMobileNav="Sports"><PageHeader eyebrow="Games hub" title="SKYBET match engine" description="View SKYBET-generated football pairings, upcoming/live states, forecasts, scores, and market odds. These are internally managed markets, not an official sports feed." /><main className="container py-5 md:py-6"><GamesFeedPreview heading="Scores and fixtures" /><Card className="mt-3 border-[var(--sky-blue-100)] bg-[var(--sky-navy-950)] text-white dark:border-white/10"><CardContent className="flex items-center gap-3 p-3"><Gamepad2 className="size-5 text-[var(--sky-blue-300)]" /><div><p className="text-sm font-extrabold">Simulation boundary</p><p className="mt-1 text-xs leading-5 text-slate-300">Pairings, dates, scores, forecasts, and market odds are generated by SKYBET’s internal market engine. They are not official fixtures, and settlement remains an explicit administrative action.</p></div></CardContent></Card></main></CustomerShell>;
}
...
export function ActivityPage() {
  ...
  const referralLink = `${typeof window === "undefined" ? "https://skybet.example" : window.location.origin}/?ref=${encodeURIComponent(referralCode)}`;
...
export function EventDetailPage() {
  ...
  const [selection, setSelection] = useState<PreviewSelection>(null);
  const [selectionOpen, setSelectionOpen] = useState(false);
  const simulationFeed = trpc.games.simulatedFeed.useQuery(undefined, { refetchInterval: 30_000 });
  const event = simulationFeed.data?.events.find(item => item.id === params?.id);
