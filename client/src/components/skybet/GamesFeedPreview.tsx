...
export function GamesFeedPreview({ heading = "ESPN match preview" }: GamesFeedPreviewProps) {
  const [category, setCategory] = useState("All");
  const scoreboard = trpc.sportsData.scoreboard.useQuery({ league: "eng.1" }, {
  ...
  return (
    <section className="mt-5" aria-labelledby="games-feed-heading">
      <div className="mb-3 flex items-end justify-between gap-3">
        ...
        <Button type="button" variant="ghost" size="icon" aria-label="Refresh ESPN match preview" className="size-10 rounded-xl text-[var(--sky-blue-700)] hover:bg-[var(--sky-ice-100)]" disabled={scoreboard.isFetching} onClick={() => scoreboard.refetch()}>
          <RefreshCw className={`size-4 ${scoreboard.isFetching ? "animate-spin" : ""}`} />
        </Button>
      </div>

      <div className="mb-3 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none]" aria-label="Match preview categories">
      ...
      <Card className="overflow-hidden border-[var(--sky-blue-100)] bg-white shadow-[0_10px_24px_rgba(10,63,158,0.05)] dark:border-white/10 dark:bg-[var(--card)]">
        <CardContent className="p-0">
          <div className="flex items-center justify-between gap-3 border-b border-[var(--sky-blue-100)] bg-[var(--sky-ice-50)] px-3 py-2 text-xs dark:border-white/10 dark:bg-white/5">
            <span className="font-bold text-[var(--sky-navy-700)] dark:text-slate-300">Data sourced from ESPN · independent preview</span>
            <span className="font-semibold text-[var(--sky-navy-500)] dark:text-slate-400">{scoreboard.data ? `Refreshes every ${scoreboard.data.refreshAfterSeconds}s` : "Loading preview"}</span>
          </div>
          <div role="status" className="flex items-start gap-2 border-b border-[var(--sky-blue-100)] px-3 py-2 text-xs leading-5 text-[var(--sky-navy-600)] dark:border-white/10 dark:text-slate-400"><CircleAlert className="mt-0.5 size-3.5 shrink-0 text-[var(--sky-blue-600)]" />{scoreboard.data?.message ?? providerStatus.data?.message}</div>
          {scoreboard.isError ? <p className="p-3 text-sm text-destructive">The ESPN match preview is unavailable. Please refresh.</p> : null}
          <div className="flex gap-2 overflow-x-auto p-2.5 pb-3 [scrollbar-width:none] sm:grid sm:grid-cols-3 sm:overflow-visible">
            ...
            {visibleEvents.map(event => (
              <article key={event.id} className="min-w-[8.5rem] rounded-lg border border-[var(--sky-blue-100)] bg-[var(--sky-white-50)] p-2 text-left dark:border-white/10 dark:bg-white/5 sm:min-w-0">
                ...
                <p className="mt-1 truncate text-[13px] font-extrabold text-[var(--sky-navy-950)] dark:text-white">{event.homeTeam}{event.homeScore !== null ? `  ${event.homeScore}` : ""}</p>
                <p className="mt-px truncate text-[13px] font-extrabold text-[var(--sky-navy-950)] dark:text-white">{event.awayTeam}{event.awayScore !== null ? `  ${event.awayScore}` : ""}</p>
                <p className="mt-1 truncate text-[11px] text-[var(--sky-navy-600)] dark:text-slate-400">{event.competition} · {event.status}</p>
                <span className="mt-1.5 block text-[11px] font-extrabold text-[var(--sky-blue-700)] dark:text-[var(--sky-blue-300)]">Scores only · not betting odds</span>
              </article>
            ))}
            {!scoreboard.isLoading && !scoreboard.isError && visibleEvents.length === 0 ? <p className="p-3 text-sm text-[var(--sky-navy-600)] dark:text-slate-400">No ESPN match updates are available in this category yet.</p> : null}
          </div>
        </CardContent>
      </Card>
    </section>
