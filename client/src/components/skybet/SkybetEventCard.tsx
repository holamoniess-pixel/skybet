  ...
  return (
    <Card className="overflow-hidden border-[var(--sky-blue-100)] bg-white shadow-[0_4px_12px_rgba(10,63,158,0.035)] dark:border-white/10 dark:bg-[var(--card)]">
      <CardContent className="p-0">
        <div className="flex items-center justify-between gap-2 border-b border-[var(--sky-blue-100)] bg-[var(--sky-ice-50)] px-2.5 py-1 dark:border-white/10 dark:bg-white/5">
          <div className="flex min-w-0 items-center gap-1.5 text-[11px] font-bold text-[var(--sky-navy-600)] dark:text-slate-300">
            <StatusDot live={event.isLive} />
            <span className="truncate">{event.competition}</span>
          </div>
          <span className="shrink-0 text-[11px] font-extrabold text-[var(--sky-blue-700)] dark:text-[var(--sky-blue-300)]">{event.startsAt}</span>
        </div>
        <div className="p-2">
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <p className="text-[13px] font-extrabold tracking-[-0.03em] text-[var(--sky-navy-950)] dark:text-white">
                {event.teams[0]} <span className="font-medium text-[var(--sky-navy-400)]">vs</span> {event.teams[1]}
              </p>
              <p className="mt-px flex items-center gap-1 text-[10px] text-[var(--sky-navy-600)] dark:text-slate-400">
                <Clock3 className="size-3" />
                {event.status}
              </p>
            </div>
            {event.score ? (
              <div className="rounded-md bg-[var(--sky-navy-950)] px-2 py-0.5 text-[13px] font-extrabold tabular-nums text-white">{event.score}</div>
            ) : (
          ...
          <div className={`mt-1.5 grid gap-1 ${displayedMarkets.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}>
            {displayedMarkets.map(market => {
              ...
              return (
                <button
                  ...
                  className={`min-h-11 rounded-md border px-1.5 py-1 text-left transition ${
                    selected
                ...
                >
                  <span className={`block truncate text-[9px] font-bold ${selected ? "text-white/75" : "text-[var(--sky-navy-600)] dark:text-slate-400"}`}>{market.label}</span>
                  <span className="mt-px block text-[13px] font-extrabold tabular-nums">{market.value}</span>
                </button>
              );
            })}
          </div>
          {hiddenMarketCount > 0 ? (
            <button
              ...
              className="mt-1.5 flex min-h-10 w-full items-center justify-center gap-1 rounded-md border border-dashed border-[var(--sky-blue-200)] px-3 text-[11px] font-extrabold text-[var(--sky-blue-700)] transition hover:bg-[var(--sky-ice-50)] dark:border-white/15 dark:text-[var(--sky-blue-300)] dark:hover:bg-white/5"
            >
