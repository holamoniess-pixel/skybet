...
export function SkybetEventCard({ event, selectedMarket, onMarketSelect }: SkybetEventCardProps) {
  return (
    <Card className="overflow-hidden border-[var(--sky-blue-100)] bg-white shadow-[0_6px_16px_rgba(10,63,158,0.04)] dark:border-white/10 dark:bg-[var(--card)]">
      <CardContent className="p-0">
        <div className="flex items-center justify-between gap-3 border-b border-[var(--sky-blue-100)] bg-[var(--sky-ice-50)] px-3 py-2 dark:border-white/10 dark:bg-white/5">
        ...
        <div className="p-3">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-[15px] font-extrabold tracking-[-0.03em] text-[var(--sky-navy-950)] dark:text-white">
            ...
            {event.score ? (
              <div className="rounded-lg bg-[var(--sky-navy-950)] px-2.5 py-1.5 text-sm font-extrabold tabular-nums text-white">{event.score}</div>
            ) : (
          ...
          <div className={`mt-2.5 grid gap-1.5 ${event.markets.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}>
            {event.markets.map(market => {
              ...
              return (
                <button
                  ...
                  className={`min-h-11 rounded-lg border px-2 py-1.5 text-left transition ${
                    selected
