...
export function SelectionSheet({ open, onOpenChange, selection, selections, onRemoveSelection, onClearSelections, onLoadCode, codeMessage }: SelectionSheetProps) {
  ...
  const handlePlaceBet = async () => {
    if (placeWager.isPending) return;
    if (!Number.isFinite(numericStake) || numericStake <= 0) {
      toast.error("Enter a valid stake before placing the bet.");
      return;
    }
    try {
      const wager = await placeWager.mutateAsync({
        idempotencyKey: typeof crypto?.randomUUID === "function" ? crypto.randomUUID() : `bet-${Date.now()}`,
        stake: numericStake,
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-[80dvh] rounded-t-[2rem] border-[var(--sky-blue-100)] bg-[var(--sky-white-50)] p-0 dark:border-white/10 dark:bg-[var(--card)] sm:mx-auto sm:max-w-lg">
        <SheetHeader className="border-b border-[var(--sky-blue-100)] p-5 text-left dark:border-white/10">
          <SheetTitle className="text-xl font-extrabold tracking-[-0.04em] text-[var(--sky-navy-950)] dark:text-white">Review your betslip</SheetTitle>
          <SheetDescription>Enter your stake and place a bet. Your deposited balance is checked on the server before deduction.</SheetDescription>
        </SheetHeader>
