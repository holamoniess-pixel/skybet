...
export function WalletPaymentRequestCard() {
  ...
  const submit = () => {
    ...
    if (mode === "deposit") {
      if (method === "aquapay") startAquaPayDeposit.mutate({ amount, customerPaymentReference: reference, mobileMoneyNumber, network: mobileMoneyNetwork });
    ...
    <div className="mt-5 space-y-5">
      ...
      {isAuthenticated && mode === "deposit" ? <div className="space-y-5">
        <div className="grid gap-2 sm:grid-cols-2">
          {methods.data?.map(item => <button key={item.method} type="button" disabled={item.status !== "enabled"} onClick={() => setMethod(item.method)} className={`rounded-xl border p-3 text-left transition ${method === item.method ? "border-[var(--sky-blue-600)] bg-[var(--sky-ice-50)]" : "border-[var(--sky-blue-100)] bg-white dark:border-white/10 dark:bg-white/5"} ${item.status !== "enabled" ? "cursor-not-allowed opacity-55" : "hover:border-[var(--sky-blue-400)]"}`}><div className="flex items-start justify-between gap-2"><span className="text-sm font-extrabold text-[var(--sky-navy-950)] dark:text-white">{item.method === "aquapay" ? "Mobile Money" : item.displayName}</span><Badge className={item.status === "enabled" ? "bg-[var(--sky-emerald-600)] hover:bg-[var(--sky-emerald-600)]" : "bg-slate-400 hover:bg-slate-400"}>{item.status === "enabled" ? "Available" : "Coming soon"}</Badge></div><p className="mt-2 text-xs leading-5 text-[var(--sky-navy-600)] dark:text-slate-400">{item.method === "crypto_trc20" ? "Send USDT on the TRC20 network, then upload your payment screenshot for review." : pendingGateway ? "Choose your network and number to continue to the Mobile Money payment page." : "Choose your network and number to continue to the Mobile Money payment page."}</p></button>)}
        </div>
        ...
        {method === "aquapay" ? <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="deposit-mobile-money-network">Mobile Money network</Label><select id="deposit-mobile-money-network" value={mobileMoneyNetwork} onChange={event => setMobileMoneyNetwork(event.target.value as typeof mobileMoneyNetwork)} className="h-11 w-full rounded-xl border border-[var(--sky-blue-200)] bg-background px-3 text-sm dark:border-white/15 dark:bg-white/5"><option value="MTN">MTN</option><option value="VODAFONE">Vodafone</option><option value="AIRTELTIGO">AirtelTigo</option></select></div><div className="space-y-2"><Label htmlFor="deposit-mobile-money-number">Mobile Money number</Label><Input id="deposit-mobile-money-number" inputMode="tel" autoComplete="tel" value={mobileMoneyNumber} onChange={event => setMobileMoneyNumber(event.target.value)} placeholder="024 123 4567 or +233 24 123 4567" className="h-11 rounded-xl border-[var(--sky-blue-200)] dark:border-white/15 dark:bg-white/5" /></div><p className="text-xs leading-5 text-[var(--sky-navy-600)] dark:text-slate-400 sm:col-span-2">After you continue, you will be redirected to AquaPay’s secure hosted payment page to complete Mobile Money payment.</p></div> : null}
        <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="payment-reference">{method === "aquapay" ? "Transfer reference (optional)" : "Your transfer reference"}</Label><Input id="payment-reference" value={reference} onChange={event => setReference(event.target.value)} placeholder={method === "aquapay" ? "Optional transaction ID" : "Transaction ID or reference"} className="h-11 rounded-xl border-[var(--sky-blue-200)] dark:border-white/15 dark:bg-white/5" />{method === "aquapay" ? <p className="text-xs leading-5 text-[var(--sky-navy-600)] dark:text-slate-400">AquaPay will generate a reference if you leave this blank.</p> : null}</div><div className="space-y-2"><Label htmlFor="payment-proof">Payment screenshot</Label><div className="flex h-11 items-center gap-2 rounded-xl border border-[var(--sky-blue-200)] px-3 dark:border-white/15"><FileImage className="size-4 text-[var(--sky-blue-700)]" /><input id="payment-proof" type="file" accept="image/png,image/jpeg" onChange={onProofChange} className="min-w-0 text-xs" /></div></div></div>
        ...
        <Button type="button" aria-busy={submitDeposit.isPending || startAquaPayDeposit.isPending} disabled={submitDisabled} onClick={submit} className="h-12 w-full rounded-xl bg-[var(--sky-blue-600)] font-extrabold text-white shadow-[0_10px_22px_rgba(15,87,199,0.22)] transition hover:bg-[var(--sky-blue-700)] active:scale-[0.98]">{method === "aquapay" ? (startAquaPayDeposit.isPending ? "Connecting to Mobile Money…" : "Continue to Mobile Money") : (submitDeposit.isPending ? "Submitting…" : "Submit deposit request for review")}</Button>
      </div> : null}
