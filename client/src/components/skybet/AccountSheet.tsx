...
const accountItems = [
  [ShieldCheck, "Safer play", "Limits, time-outs, and account protection"],
  [Heart, "Referral rewards", "View eligible referral progress"],
  [Copy, "Activity", "Review account and selection history"],
  [Sparkles, "Preferences", "Personalise the Skybet experience"],
] as const;
      ...
      <SheetContent side="right" className="w-[92%] border-[var(--sky-blue-100)] bg-[var(--sky-white-50)] p-0 dark:border-white/10 dark:bg-[var(--card)] sm:max-w-md">
        ...
        <div className="space-y-3 p-5">
          <div className="rounded-2xl bg-[var(--sky-navy-950)] p-4 text-white">
            ...
            <Button className="mt-4 h-10 w-full rounded-xl bg-white font-extrabold text-[var(--sky-blue-700)] hover:bg-[var(--sky-ice-50)]" onClick={() => toast.message("Authentication will be activated in the secured Skybet release.")}>Sign in to Skybet</Button>
          </div>
          {accountItems.map(([Icon, title, description]) => (
            <button key={title} type="button" onClick={() => toast.message(`${title} is mapped for the next approved Skybet release.`)} className="flex min-h-[4.5rem] w-full items-center gap-3 rounded-2xl border border-[var(--sky-blue-100)] bg-white p-3 text-left transition hover:border-[var(--sky-blue-300)] hover:bg-[var(--sky-ice-50)] dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10">
